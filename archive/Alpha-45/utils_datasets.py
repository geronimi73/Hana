...
class PD12MShapeBatchingDataset(torch.utils.data.Dataset):
    def __init__(
        self, hf_dataset, 
        text_enc, tokenizer, 
        bs, ddp=False, device=None, dtype=None, 
    ...
    def prepare_batch(self, items, shape):
        ...
        # remove weird stuff from labels
        for i in range(len(labels)):
            labels[i] = labels[i].replace("The image shows","").strip()
            labels[i] = labels[i].replace("In this image we can see","").strip()
            labels[i] = labels[i][:-1] if labels[i].endswith('.') else labels[i]
