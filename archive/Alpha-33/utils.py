...
def kl_encode(img, vae):
    img_tensor = T.transforms.ToTensor()(img)[None].to(vae.dtype).to(vae.device)
...
def kl_decode(latent, vae):
    with torch.no_grad():
        image_out = vae.decode(latent).sample.cpu()
    
    if image_out.size(0) == 1:
        img = torch.clamp_(image_out[0,:], -1, 1)
        img = img * 0.5 + 0.5
        return T.ToPILImage()(img.float())
    else:
        images = []
